...
# ============================================================
# Tee：同时写stdout和log文件（UTF-8，行缓冲）。仅IO工具，不影响训练逻辑
# ============================================================
class _Tee:
...
# 程序启动立刻挂钩（在任何import/torch初始化之前打印，确保可追溯）
_log_p = os.path.join(_HERE, 'runs', 'mds_power_comparison_log.txt')
os.makedirs(os.path.dirname(_log_p), exist_ok=True)
sys.stdout = _Tee(_log_p)
...
# ============================================================
# MDS 方法 P1: 一次方 (d/d0) + (d0/d)
# ============================================================
def mds_p1(distances: torch.Tensor, q_dim: int) -> torch.Tensor:
    n = distances.shape[0]
    if n <= q_dim:
...
# ============================================================
# MDS 方法 P2: 二次方 (d/d0)^2 + (d0/d)^2
# ============================================================
def mds_p2(distances: torch.Tensor, q_dim: int) -> torch.Tensor:
    n = distances.shape[0]
    if n <= q_dim:
        return torch.randn(n, q_dim, dtype=distances.dtype, device=distances.device) * 0.1
    high_dim, low_dim = n, q_dim
...
# ============================================================
# MDS 方法 P3: 三次方 (d/d0)^3 + (d0/d)^3
# ============================================================
def mds_p3(distances: torch.Tensor, q_dim: int) -> torch.Tensor:
    n = distances.shape[0]
    if n <= q_dim:
        return torch.randn(n, q_dim, dtype=distances.dtype, device=distances.device) * 0.1
    high_dim, low_dim = n, q_dim
...
def run_one_dataset(train_ds, test_ds, mds_func, mds_name: str, config: LatentQConfig):
    ...
    # 训练（与第一个实验100%一致：无梯度裁剪、无NaN检查、无进度打印）
    t0 = time.time()
...
def main():
    ...
    mds_methods = [
        ("P1_一次方(d/d0+d0/d)", mds_p1),
        ("P2_二次方(d/d0)2+(d0/d)2", mds_p2),
        ("P3_三次方(d/d0)3+(d0/d)3", mds_p3),
    ]

    # 与第一个实验 run_mds_comparison.py 100%一致：epochs=1000, 其他参数完全相同
    ...
    all_results = []
    print("=" * 80, flush=True)
    print("MDS损失函数幂次对比测试: 一次方 vs 二次方 vs 三次方 (q_dim=3, Q固定后只训练θ)", flush=True)
    print("(控制变量：epochs=1000 与第一个实验完全一致)", flush=True)
    print("=" * 80, flush=True)
    ...
    print(f"\n\n{'=' * 80}", flush=True)
    print("                         ╔══════════════════════════════════════════╗", flush=True)
    print("                         ║   一/二/三 次 方 损 失 对 比 结 果       ║", flush=True)
    print("                         ╚══════════════════════════════════════════╝", flush=True)
    print(f"{'=' * 80}", flush=True)
    ...
    if all_results:
        ...
        for ds in pivot_r2.index:
            ...
            for col in pivot_r2.columns:
                ...
                print(f"    {col:35s} →  R² = {vals[col]:.4f}{marker}", flush=True)
        ...
        for ds in p_tr.index:
            ...
            for col in p_tr.columns:
                print(f"    {col:35s} →  R² = {p_tr.loc[ds, col]:.4f}", flush=True)
        ...
        for ds in p_sp.index:
            ...
            for col in p_sp.columns:
                print(f"    {col:35s} →  ρ = {p_sp.loc[ds, col]:.4f}", flush=True)
        ...
        for ds in df['dataset'].unique():
            ...
            summary_rows.append({
                ...
                'P1测试R²': f"{sub[sub['mds_name']==mds_methods[0][0]]['test_r2'].values[0]:.4f}" if len(sub[sub['mds_name']==mds_methods[0][0]])>0 else 'N/A',
                'P2测试R²': f"{sub[sub['mds_name']==mds_methods[1][0]]['test_r2'].values[0]:.4f}" if len(sub[sub['mds_name']==mds_methods[1][0]])>0 else 'N/A',
                'P3测试R²': f"{sub[sub['mds_name']==mds_methods[2][0]]['test_r2'].values[0]:.4f}" if len(sub[sub['mds_name']==mds_methods[2][0]])>0 else 'N/A',
                'P1训练R²': f"{sub[sub['mds_name']==mds_methods[0][0]]['train_r2'].values[0]:.4f}" if len(sub[sub['mds_name']==mds_methods[0][0]])>0 else 'N/A',
                'P2训练R²': f"{sub[sub['mds_name']==mds_methods[1][0]]['train_r2'].values[0]:.4f}" if len(sub[sub['mds_name']==mds_methods[1][0]])>0 else 'N/A',
                'P3训练R²': f"{sub[sub['mds_name']==mds_methods[2][0]]['train_r2'].values[0]:.4f}" if len(sub[sub['mds_name']==mds_methods[2][0]])>0 else 'N/A',
            })
        ...
        out = os.path.join(_HERE, 'runs', 'mds_power_comparison_final.csv')
        df.to_csv(out, index=False, encoding='utf-8-sig')
        sum_out = os.path.join(_HERE, 'runs', 'mds_power_comparison_summary.csv')
        ...
        print("   P1 (一次方): 损失=(d/d0)+(d0/d)，梯度温和，对距离失配惩罚线性增长", flush=True)
        print("   P2 (二次方): 损失=(d/d0)²+(d0/d)²，惩罚适中，梯度平滑", flush=True)
        print("   P3 (三次方): 损失=(d/d0)³+(d0/d)³，惩罚陡峭，对大失配极度敏感", flush=True)
        ...
        for c in p_sp.columns: print(f"     {c:35s}: {sp_mean[c]:.4f}", flush=True)
        print(f"   方法平均 测试R²: ", flush=True)
        for c in pivot_r2.columns: print(f"     {c:35s}: {r2_mean[c]:.4f}", flush=True)
        ...
        for k, v in wins.items():
            print(f"   {k:35s} : 赢{v}次", flush=True)
