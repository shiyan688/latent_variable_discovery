#!/usr/bin/env python3
"""Run the frozen nine-job attentive reliability-selector pilot."""
...
METHOD = "attentive_reliability_selector"
SEEDS = (0, 1, 2)
...
def _task_id(dataset: str, seed: int, q_dim: int) -> str:
    raw = json.dumps([dataset, METHOD, seed, q_dim], separators=(",", ":")).encode()
    return f"attentive_selector_{hashlib.sha256(raw).hexdigest()[:14]}"


def _command(
    root: Path, summary: Path, dataset: str, seed: int, q_dim: int
) -> tuple[str, ...]:
    return (
        str(PYTHON),
        ...
        "128,128",
        "--support-scale-floor-fraction",
        "0.05",
        "--support-target-clip",
        "8.0",
        "--smooth-l1-beta",
        "1.0",
        "--standardized-output-bound",
        "8.0",
        "--selector-ratio",
        "0.25",
        "--selector-min-rows",
...
def build_tasks(root: Path) -> list[base.Task]:
    return [
        base.Task(
            _task_id(dataset, seed, q_dim),
            "attentive_selector",
            dataset,
            METHOD,
            seed,
            q_dim,
            _command(root, summary, dataset, seed, q_dim),
        )
...
def _write_manifest(
    tasks: list[base.Task], args: argparse.Namespace, gpus: list[str]
) -> None:
    ...
    base._write_json_atomic(
        path,
        {
            "protocol": "ICLR_ATTENTIVE_RELIABILITY_SELECTOR_PLAN_20260811.md",
            "created_at": created,
            "initial_created_at": previous.get(
                "initial_created_at", previous.get("created_at", created)
            ),
            ...
            "seeds": list(SEEDS),
            "gpus": gpus,
            "prior_anchor_root": str(
                PROJECT_ROOT / "runs" / "iclr_support_robustness_20260811"
            ),
            "dispatch_policy": "run all nine frozen jobs; no automatic retry",
            "monitoring": {
    ...
    with (args.output_root / "planned_tasks.jsonl").open(
        "w", encoding="utf-8"
    ) as handle:
        for task in tasks:
...
def run_campaign(args: argparse.Namespace) -> int:
    ...
    while pending or running:
        ...
        for gpu in gpus:
            if (
                not pending
                or gpu in running
                or memory.get(gpu, 10**9) >= args.gpu_memory_threshold_mib
            ):
                continue
            task = pending.pop(0)
            ...
            handle.write(
                f"[{started_at}] CUDA_VISIBLE_DEVICES={gpu} {' '.join(task.command)}\n"
            )
            handle.flush()
        ...
        base._write_json_atomic(
            status_path, base._status_payload(tasks, terminal, running, "running")
        )
        if pending or running:
            time.sleep(max(1, args.poll_seconds))
    state = (
        "completed_all"
        if all(row.get("event") == "success" for row in terminal.values())
    ...
    with analysis_log.open("w", encoding="utf-8") as handle:
        analysis = subprocess.run(
            [
                str(PYTHON),
                "scripts/analyze_attentive_selector_20260811.py",
                "--campaign-root",
    ...
    final_status["analysis"] = {
        ...
        "report": str(args.output_root / "ATTENTIVE_SELECTOR_RESULTS.md"),
