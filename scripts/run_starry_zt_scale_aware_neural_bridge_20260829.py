#!/usr/bin/env python3
"""Run one frozen fold/seed cell of the scale-aware Starry ZT bridge repair."""
...
FORMAL_ROOT = PROJECT_ROOT / "runs/starry_zt_scale_aware_neural_bridge_20260829"
...
def predict_model(
    ...
    transformed = denormalize_targets(prediction.cpu().numpy(), artifacts.normalizer)
    return target_transform_scale * np.sinh(transformed)
...
def main() -> None:
    ...
    target_transform_scale = float(
        outer_train.groupby("label", sort=True)["target"].std(ddof=0).median()
    )
    outer_train["model_target"] = np.arcsinh(
        outer_train["target"] / target_transform_scale
    )
    outer_test["model_target"] = np.arcsinh(
        outer_test["target"] / target_transform_scale
    )
    ...
    manifest = {
        "scope": "starry_zt_scale_aware_neural_bridge_cell",
        ...
        "target_transform": "asinh_train_entity_median_population_std",
        "target_transform_scale": target_transform_scale,
        "temporal_confirmation_opened": False,
    }
    write_json(cell_root / "manifest.running.json", manifest)

    selected_columns = ["label", *FEATURES, "model_target"]
    ...
    normalized_test_targets = torch.tensor(
        normalize_targets(
            outer_test["model_target"].to_numpy(np.float32), artifacts.normalizer
        ).reshape(-1, 1),
        dtype=torch.float32,
        device=artifacts.device,
    )
    ...
    for label, entity in outer_test.groupby("label", sort=True):
        ...
        decoder_response = predict_model(
            model, grid_features, calibrated_q, artifacts, target_transform_scale
        )
