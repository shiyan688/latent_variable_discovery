#!/usr/bin/env python3
"""Run the frozen 27-job support-relative robustness pilot."""
...
def _task_id(dataset: str, method: str, seed: int, q_dim: int) -> str:
    ...
    return f"support_robustness_{hashlib.sha256(raw).hexdigest()[:14]}"
    ...
    for dataset, summary, q_dim in DATASETS:
        for method in METHODS:
            for seed in SEEDS:
                tasks.append(
                    base.Task(
                        _task_id(dataset, method, seed, q_dim),
                        "support_robustness",
                        dataset,
                        method,
                        seed,
                        q_dim,
                        _command(root, summary, dataset, method, seed, q_dim),
                    )
                )
    return tasks
...
def _write_manifest(
    ...
    base._write_json_atomic(
        path,
        {
            "protocol": "ICLR_SUPPORT_RELATIVE_ROBUSTNESS_PLAN_20260811.md",
            "created_at": created,
            ...
            "prior_anchor_root": str(
                PROJECT_ROOT / "runs" / "iclr_support_followup_20260811"
            ),
            "dispatch_policy": (
                "run all 27 frozen jobs; never auto-retry terminal failures"
            ),
            "monitoring": {
...
def run_campaign(args: argparse.Namespace) -> int:
    ...
    with analysis_log.open("w", encoding="utf-8") as handle:
        analysis = subprocess.run(
            [
                str(PYTHON),
                "scripts/analyze_support_robustness_20260811.py",
                "--campaign-root",
    ...
    final_status["analysis"] = {
        ...
        "report": str(args.output_root / "SUPPORT_ROBUSTNESS_RESULTS.md"),
